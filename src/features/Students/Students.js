...
const Students = () => {
    ...
    if(isSuccess){
        ...
        let filteredIds = ids
        if(params){
            filteredIds = ids.filter(id => students.entities[id]?.matricNo.toLowerCase().includes(params) || students.entities[id]?.name?.firstName.toLowerCase().includes(params) || students.entities[id]?.name?.surname.toLowerCase().includes(params) || students.entities[id]?.name?.otherName.toLowerCase().includes(params) || students.entities[id]?.gender.toLowerCase().includes(params) || students.entities[id]?.contact?.phone.toLowerCase().includes(params) || students.entities[id]?.level.toLowerCase().includes(params))
        }
            ...
            content = (
                <>
                    <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-4 px-4'>
                        {currentIds?.length > 0 && currentIds.map(studentId => <Student key={studentId} studentId={studentId} />)}
                    </div>
    ...
    <div className='container mx-auto bg-white text-gray-800 rounded-lg border shadow-lg w-full'>
        <div className='border-b border-gray-200 pb-2 flex items-center justify-between'>
            <h2 className='text-xl font-bold text-gray-700 hover:text-gray-400 p-2 w-[70%]'>
               {'Students of the Departments of Computer Science '} </h2>
            ...
            {status === 'Admin' && <Link className='bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-2 w-8 h-8
                shadow-lg rounded-full text-center mx-4 my-4 text-center' to={`/dashboard/students/new`} title='Book a student'>
